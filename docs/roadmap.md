...
## Current milestone: manual mail-to-reminder workflow

Use a small local adapter backed by Apple's EventKit. Aion keeps user-facing tools and permissions; EventKit handles system reminder storage and access. The user-directed milestone is mail reading → action/time extraction → user copies selected items → preview → terminal-confirmed creation. Implemented entry points are documented in manual-mail-reminders.md; real new-data tests remain separately scoped. Completion, deletion and other mutations need separate explicit scopes. Do not assume macOS full-access authorization grants the model every action.
...
## After manual acceptance: scheduling

Only after manual workflow acceptance, evaluate timed execution first. The branch objective is automatic email triage to reminders, with durable processed-message tracking, duplicate prevention, explicit timezone handling, scoped write policy and failure reporting. The Mac-local EventKit bridge must be reachable at runtime. No schedule or background daemon has been installed.
